...
def run_simulation(args):
    ...
    out_csv = f"outputs/2way-single-intersection/"
    ...
    for run in range(1, args.runs + 1):
        ...
        env.close()
        extract_last_iteration_values(out_csv)

def extract_last_iteration_values(csv_folder):
    input_csv_file = os.path.join(csv_folder, "_conn0_ep1.csv")
    input_file_name = os.path.splitext(os.path.basename(input_csv_file))[0]
    output_file_name = f"values_{input_file_name}.yaml"
    last_iteration_yaml = os.path.join(os.path.dirname(input_csv_file), output_file_name)
    ...
    try:
        ...
        with open(last_iteration_yaml, "w") as yamlfile:
            yaml.dump(last_iteration_values, yamlfile)

        print("Last iteration values saved successfully in:", last_iteration_yaml)
